...
const WHATSAPP_PHONE = "5491123456789"; // Format: CountryCode + AreaCode + Number (No +, No spaces)
...
// Generate Time Slots with Pseudo-random Occupied States (Realistic Premium Experience)
function loadTimeSlots(date) {
    dom.morningSlots.innerHTML = "";
    dom.afternoonSlots.innerHTML = "";
    
    // Use the date number as a seed to keep occupied slots consistent if user clicks back/forth on the same date
    ...
    const fillGrid = (slots, container, period) => {
        slots.forEach((time, index) => {
            ...
            // Simulating busy hours (e.g. index fits certain modulo based on seed)
            const isBusy = (index + dateSeed) % 4 === 0 || (index * dateSeed) % 7 === 1;
...
- 📝 Notas: ${client.notes || 'Sin notas adicionales'}`;
    ...
    const waDesktopUrl = `https://web.whatsapp.com/send?phone=${WHATSAPP_PHONE}&text=${encodedMessage}`;
    
    // Fallback URL (smart redirection for wa.me)
    ...
    setTimeout(() => {
        // Detect if mobile browser to use the app link, otherwise use fallback
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        const redirectUrl = isMobile ? waMobileUrl : waFallbackUrl;
        
        window.open(redirectUrl, '_blank');
        
        // Keep screen visible but stop active spinner/dots
